  ...
  describe("Requesting an item", () => {
    ...
    describe("As an authenticated user", () => {
      ...
      it("should be possible to request an item for loan from BIB", () => {
        ...
        cy.get("input[type=radio][name=pickup_location]").should(
          "have.length",
          1,
        );
      ...
      it("should not be possible to request a dummy barcode item via locker", () => {
        ...
        cy.location("pathname").should(
          "eq",
          "/en/catalog/rug01:000033939/items/34839-10/requests/new",
        );
        cy.param("scan").should("be.null");
        cy.param("service").should("eq", "LOAN");

        cy.get("#content > h2").should("have.text", "Prepare for loan");
        cy.get(".meta-location").should(
          "contain",
          "Location in depot: RBIB.ARCHIEF 06901",
        );

        cy.get(
          "input[type=radio][name=pickup_location][data-locker=true]",
        ).should("not.exist");
      });
